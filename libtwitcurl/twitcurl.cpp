 ...
 *
 *--*/
bool twitCurl::isCurlInit() { return (NULL != m_curlHandle) ? true : false; }
 ...
 *
 *--*/
std::string& twitCurl::getTwitterPassword() { return m_twitterPassword; }
 ...
 *
 *--*/
std::string& twitCurl::getProxyServerIp() { return m_proxyServerIp; }
 ...
 *
 *--*/
std::string& twitCurl::getProxyServerPort() { return m_proxyServerPort; }
 ...
 *
 *--*/
std::string& twitCurl::getProxyUserName() { return m_proxyUserName; }
 ...
 *
 *--*/
std::string& twitCurl::getProxyPassword() { return m_proxyPassword; }
 ...
 *
 *--*/
std::string& twitCurl::getInterface() { return m_Interface; }
                      ...
                      const std::string resultCount)
{
    ...
    std::string buildUrl =
        ...
        twitCurlDefaults::TWITCURL_SEARCHQUERYSTRING + searchQuery;
                            ...
                            const std::string inReplyToStatusId)
{
    ...
    std::string newStatusMsg =
        twitCurlDefaults::TWITCURL_STATUSSTRING + urlencode(newStatus);
 ...
 * @input: statusId - a number in std::string format
 *
 * @output: true if RETWEET is success, otherwise false. This does not check
 *http response by twitter. Use getLastWebResponse() for that.
 *
 *--*/
bool twitCurl::retweetById(const std::string& statusId)
{
    ...
    std::string dummyData =
        twitCurlDefaults::TWITCURL_TEXTSTRING + urlencode(std::string("dummy"));
...
bool twitCurl::timelineHomeGet(const std::string sinceId)
{
    ...
    if (sinceId.length()) {
        buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_QUES +
                    twitCurlDefaults::TWITCURL_SINCEID + sinceId;
    }
 ...
 *
 *--*/
bool twitCurl::timelineUserGet(const bool trimUser, const bool includeRetweets,
                               ...
                               const std::string userInfo, const bool isUserId)
{
    ...
    if (tweetCount) {
        ...
        if (tweetCount < twitCurlDefaults::MAX_TIMELINE_TWEET_COUNT) {
            tmpStrm << twitCurlDefaults::TWITCURL_URL_SEP_AMP +
            ...
            tmpStrm << twitCurlDefaults::TWITCURL_URL_SEP_AMP +
                           twitCurlDefaults::TWITCURL_COUNT
                    << twitCurlDefaults::MAX_TIMELINE_TWEET_COUNT;
        }
    ...
    if (includeRetweets) {
        buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP +
                    twitCurlDefaults::TWITCURL_INCRETWEETS;
    }
 ...
 *
 *--*/
bool twitCurl::userLookup(const std::vector<std::string>& userInfo,
                          const bool isUserId)
{
    if (userInfo.empty()) {
        return false;
    ...
    std::string userIds = "";
    std::string sep = "";
    for (unsigned int i = 0; i < std::min((size_t)100, userInfo.size());
         i++, sep = ",") {
        userIds += sep + userInfo[i];
    }

    userIds = (isUserId ? twitCurlDefaults::TWITCURL_USERID
                        : twitCurlDefaults::TWITCURL_SCREENNAME) +
              urlencode(userIds);
 ...
 *
 * @input: userInfo - screen name or user id of a user to whom message needs to
 *be sent, dMsg - message isUserId - true if userInfo contains target user's id
 *
 * @output: true if POST is success, otherwise false. This does not check http
 *          response by twitter. Use getLastWebResponse() for that.
 *
 *--*/
bool twitCurl::directMessageSend(const std::string& userInfo,
                                 const std::string& dMsg, const bool isUserId)
{
 ...
 *
 *--*/
bool twitCurl::friendshipCreate(const std::string& userInfo,
                                const bool isUserId)
{
    if (userInfo.empty()) {
        return false;
 ...
 *
 * @description: method to delete a twitter user from friend list (unfollow a
 *user)
 *
 * @input: userInfo - user id or screen name of a user
 *         isUserId - true if userInfo contains a user id instead of screen name
 ...
 *
 *--*/
bool twitCurl::friendshipDestroy(const std::string& userInfo,
                                 const bool isUserId)
{
    if (userInfo.empty()) {
        return false;
 ...
 *
 * @input: userInfo - user id or screen name of a user of whom friends need to
 *be shown isUserId - true if userInfo contains a user id instead of screen name
 *
 * @output: true if GET is success, otherwise false. This does not check http
 *          response by twitter. Use getLastWebResponse() for that.
 *
 ...
 *
 *--*/
bool twitCurl::friendsIdsGet(const std::string& nextCursor,
                             const std::string& userInfo, const bool isUserId)
{
    /* Prepare URL */
    std::string buildUrl;
    ...
    if (buildUrl.length() && nextCursor.length()) {
        buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP +
                    twitCurlDefaults::TWITCURL_NEXT_CURSOR + nextCursor;
    }
 ...
 *
 *--*/
bool twitCurl::followersIdsGet(const std::string& nextCursor,
                               const std::string& userInfo, const bool isUserId)
{
    /* Prepare URL */
    std::string buildUrl;
 ...
 *
 * @description: method to get information on user identified by given
 *credentials
 *
 * @input: none
 *
 ...
 *         skipStatus - indicates whether or not to include status for returned
 *users nextCursor - next cursor string returned from a previous call to this
 *API, otherwise an empty string
 *
 * @output: true if GET is success, otherwise false. This does not check http
 *          response by twitter. Use getLastWebResponse() for that.
 *
 *--*/
bool twitCurl::blockListGet(const std::string& nextCursor,
                            const bool includeEntities, const bool skipStatus)
{
    /* Prepare URL */
    ...
    if (includeEntities) {
        urlParams +=
            twitCurlDefaults::TWITCURL_INCLUDE_ENTITIES + std::string("true");
    }
    if (skipStatus) {
        if (urlParams.length()) {
        ...
        urlParams +=
            twitCurlDefaults::TWITCURL_SKIP_STATUS + std::string("true");
    }
 ...
 *
 *--*/
bool twitCurl::blockIdsGet(const std::string& nextCursor,
                           const bool stringifyIds)
{
    /* Prepare URL */
    std::string buildUrl, urlParams;
    ...
    if (stringifyIds) {
        urlParams +=
            twitCurlDefaults::TWITCURL_STRINGIFY_IDS + std::string("true");
    }
    if (nextCursor.length()) {
        if (urlParams.length()) {
            urlParams += twitCurlDefaults::TWITCURL_URL_SEP_AMP;
 ...
 *
 * @description: method to retrieve the data for a saved search owned by the
...
bool twitCurl::savedSearchCreate(const std::string& query)
{
    ...
    /* Send some dummy data in POST */
    std::string queryStr =
        twitCurlDefaults::TWITCURL_QUERYSTRING + urlencode(query);
 ...
 *
 * @description: method to destroy a saved search for the authenticated user.
 ...
 *
 * @input: outWebResp - string in which twitter's response is supplied back to
 *caller
 *
 * @output: none
 *
...
{
    m_errorBuffer[twitCurlDefaults::TWITCURL_DEFAULT_BUFFSIZE - 1] =
        twitCurlDefaults::TWITCURL_EOS;
 ...
 *
 *--*/
int twitCurl::curlCallback(char* data, size_t size, size_t nmemb,
 ...
 *
 * @description: method to set proxy details into cURL. this is an internal
 *method. twitcurl users should not use this method, instead use setProxyXxx
...
void twitCurl::prepareCurlProxy()
{
    ...
    if (m_proxyUserName.length() && m_proxyPassword.length()) {
        ...
        utilMakeCurlParams(proxyUserPass, getProxyUserName(),
                           getProxyPassword());
 ...
 *
 * @description: method to set callback details into cURL. this is an internal
 *method. twitcurl users should not use this method.
 *
 * @input: none
 *
 * @output: none
 ...
 *
 * @description: method to set twitter credentials into cURL. this is an
 *internal method. twitcurl users should not use this method, instead use
 *setTwitterXxx methods to set twitter username and password.
 *
 * @input: none
 *
 * @output: none
...
void twitCurl::prepareCurlUserPass()
{
    ...
    if (getTwitterUsername().size()) {
        ...
        utilMakeCurlParams(userNamePassword, getTwitterUsername(),
                           getTwitterPassword());

        /* Set username and password */
        curl_easy_setopt(m_curlHandle, CURLOPT_USERPWD,
                         userNamePassword.c_str());
    }
 ...
 *
 * @description: method to set standard params into cURL. this is an internal
 *method. twitcurl users should not use this method.
 *
 * @input: none
 *
...
bool twitCurl::performGet(const std::string& getUrl)
{
    ...
    /* Set OAuth header */
    m_oAuth.getOAuthHeader(eOAuthHttpGet, getUrl, dataStrDummy,
                           oAuthHttpHeader);
...
bool twitCurl::performDelete(const std::string& deleteUrl)
{
    ...
    /* Set OAuth header */
    m_oAuth.getOAuthHeader(eOAuthHttpDelete, deleteUrl, dataStrDummy,
                           oAuthHttpHeader);
    if (oAuthHttpHeader.length()) {
        pOAuthHeaderList =
            curl_slist_append(pOAuthHeaderList, oAuthHttpHeader.c_str());
    ...
    curl_easy_setopt(m_curlHandle, CURLOPT_COPYPOSTFIELDS,
                     dataStrDummy.c_str());
 ...
 *         in this string. user should visit this link and get PIN from that
 *page.
 *
...
bool twitCurl::oAuthRequestToken(std::string& authorizeUrl /* out */)
{
    ...
    authorizeUrl = "";
    if (m_oAuth.getOAuthHeader(
            eOAuthHttpGet,
            twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
                oAuthTwitterApiUrls::OAUTHLIB_TWITTER_REQUEST_TOKEN_URL,
            std::string(""), oAuthHeader)) {
        if (performGetInternal(
                twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
                    oAuthTwitterApiUrls::OAUTHLIB_TWITTER_REQUEST_TOKEN_URL,
                oAuthHeader)) {
            /* Tell OAuth object to save access token and secret from web
             * response */
...
bool twitCurl::oAuthHandlePIN(const std::string& authorizeUrl /* in */)
{
    ...
    /* Set OAuth header */
    m_oAuth.getOAuthHeader(eOAuthHttpGet, authorizeUrl, dataStr,
                           oAuthHttpHeader);
    if (oAuthHttpHeader.length()) {
        pOAuthHeaderList =
            curl_slist_append(pOAuthHeaderList, oAuthHttpHeader.c_str());
    ...
    if (CURLE_OK == curl_easy_perform(m_curlHandle)) {
        if (pOAuthHeaderList) {
            curl_easy_getinfo(m_curlHandle, CURLINFO_HTTP_CODE,
                              &httpStatusCode);
            curl_slist_free_all(pOAuthHeaderList);

            // Now, let's find the authenticity token and oauth token
            nPosStart = m_callbackData.find(
                oAuthLibDefaults::OAUTHLIB_AUTHENTICITY_TOKEN_TWITTER_RESP_KEY);
            ...
            nPosStart +=
                oAuthLibDefaults::OAUTHLIB_AUTHENTICITY_TOKEN_TWITTER_RESP_KEY
                    .length();
            nPosEnd = m_callbackData.substr(nPosStart).find(
                oAuthLibDefaults::OAUTHLIB_TOKEN_END_TAG_TWITTER_RESP);
            ...
            nPosStart = m_callbackData.find(
                oAuthLibDefaults::OAUTHLIB_TOKEN_TWITTER_RESP_KEY);
            if (std::string::npos == nPosStart) {
                return false;
            }
            nPosStart +=
                oAuthLibDefaults::OAUTHLIB_TOKEN_TWITTER_RESP_KEY.length();
            nPosEnd = m_callbackData.substr(nPosStart).find(
                oAuthLibDefaults::OAUTHLIB_TOKEN_END_TAG_TWITTER_RESP);
            if (std::string::npos == nPosEnd) {
                return false;
    ...
    */
    dataStr = oAuthLibDefaults::OAUTHLIB_TOKEN_KEY + "=" + oauthTokenVal + "&" +
              oAuthLibDefaults::OAUTHLIB_AUTHENTICITY_TOKEN_KEY + "=" +
              authenticityTokenVal + "&" +
              oAuthLibDefaults::OAUTHLIB_SESSIONUSERNAME_KEY + "=" +
              getTwitterUsername() + "&" +
              oAuthLibDefaults::OAUTHLIB_SESSIONPASSWORD_KEY + "=" +
              getTwitterPassword();

    /* Set OAuth header */
    m_oAuth.getOAuthHeader(eOAuthHttpPost, authorizeUrl, dataStr,
                           oAuthHttpHeader);
    if (oAuthHttpHeader.length()) {
        pOAuthHeaderList =
            curl_slist_append(pOAuthHeaderList, oAuthHttpHeader.c_str());
    ...
    if (CURLE_OK == curl_easy_perform(m_curlHandle)) {
        if (pOAuthHeaderList) {
            ...
            nPosStart = m_callbackData.find(
                oAuthLibDefaults::OAUTHLIB_PIN_TWITTER_RESP_KEY);
            if (std::string::npos == nPosStart) {
                return false;
            }
            nPosStart +=
                oAuthLibDefaults::OAUTHLIB_PIN_TWITTER_RESP_KEY.length();
            nPosEnd = m_callbackData.substr(nPosStart).find(
                oAuthLibDefaults::OAUTHLIB_PIN_END_TAG_TWITTER_RESP);
            if (std::string::npos == nPosEnd) {
                return false;
            }
...
size_t userstream_curl_callback(void* ptr, size_t size, size_t nmemb,
                                void* data)
{
    auto* callback =
        reinterpret_cast<std::function<void(const std::string&)>*>(data);
    size_t realsize = size * nmemb;
    if (realsize == 0) return 0;
    (*callback)(std::string(static_cast<char*>(ptr), realsize));
    return realsize;
}

void twitCurl::usertimeline(std::function<void(const std::string&)> func)
{
    const static std::string getUrl =
    ...
    [&]() {
        std::string dummy, oAuthHttpHeader;
        m_oAuth.getOAuthHeader(eOAuthHttpGet, getUrl, dummy, oAuthHttpHeader);
        if (oAuthHttpHeader.length())
            headers = curl_slist_append(headers, oAuthHttpHeader.c_str());
    }();
    ...
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, userstream_curl_callback);
